...
public class ProjectHeaderContainer extends VisTable implements Listener1<String> {
	...
	public ProjectHeaderContainer() {
		...
		saveButton.addListener(new ChangeListener() {
			@Override
			public void changed(ChangeEvent event, Actor actor) {
				EventService.notify(GdxStudioEvents.SAVE_PROJECT_EVENT);
			}
		});
		...
		EventService.addListener(GdxStudioEvents.SELECT_PROJECT_EVENT, this);
	}
	...
	private class SelectionChangeListener extends ChangeListener {
		...
		public void changed(ChangeEvent event, Actor actor) {
			...
			if (selected != lastSelected) {
				...
				EventService.notify(new SceneSelectionChangedEvent(selected.scene));
			}
		}
	}
