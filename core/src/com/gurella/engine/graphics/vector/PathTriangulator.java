...
abstract class PathTriangulator implements PathConstants {
	...
	protected void roundJoin(Array<Vertex> dst, Point p0, Point p1, float lw, float rw, float lu, float ru, int ncap) {
		...
		if ((p1.flags & PT_LEFT) != 0) {
			...
			a0 = MathUtils.atan2(-dly0, -dlx0);
			a1 = MathUtils.atan2(-dly1, -dlx1);
			...
			int n = MathUtils.clamp((int) Math.ceil(((a0 - a1) / MathUtils.PI) * ncap), 2, ncap);
			for (int i = 0; i < n; i++) {
				float u = i / (float) (n - 1);
				float a = a0 + u * (a1 - a0);
				float rx = p1.x + MathUtils.cos(a) * rw;
				float ry = p1.y + MathUtils.sin(a) * rw;
			...
			int n = MathUtils.clamp((int) Math.ceil(((a1 - a0) / MathUtils.PI) * ncap), 2, ncap);
			for (int i = 0; i < n; i++) {
				float u = i / (float) (n - 1);
				float a = a0 + u * (a1 - a0);
				float lx = p1.x + MathUtils.cos(a) * lw;
				float ly = p1.y + MathUtils.sin(a) * lw;
