...
public abstract class OpenTypeLayoutTable extends SfntTable {
	...
	OpenTypeLayoutTable(RandomAccessFile raf, TableDirectory directoryTable, int tag, long checkSum, int offset,
			int length) {
	...
	private void parseLookups(int lookupListOffset) {
		...
		for (int i = 0; i < count; i++) {
			...
			OpenTypeLookupTable subTable = new OpenTypeLookupTable(this, listOffset + subTableOffset,
