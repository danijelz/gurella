...
public class MainView2 extends Composite {
	...
	public MainView2(Composite parent, int style) {
		super(parent, style);

		Display display = getDisplay();
		testImage = new Image(display, ImageDescriptor.createFromFile(MainView2.class, "palette_view.gif").getImageData());
	...
	public void layout(boolean changed) {
		...
		int southDockCenterHeight = south.isContentVisible() && south.isMinimized() ? CLOSED_DOCK_EXTENT : southDockHeight;

		center.setBounds(clientArea.x + westDockCenterWidth + 2, clientArea.y, clientArea.width - westDockCenterWidth - eastDockCenterWidth - 4,
		...
		east.setBounds(clientArea.width - eastDockWidth, clientArea.y, eastDockWidth, clientArea.height - southDockCenterHeight - 2);
	...
	private static class DockComponent extends Composite {
		...
		public DockComponent(MainView2 parent, int position) {
		...
		@Override
		public MainView2 getParent() {
			return (MainView2) super.getParent();
		}
		...
		protected void createSash() {
			...
			sash.setCursor(getDisplay().getSystemCursor(position == SWT.BOTTOM ? SWT.CURSOR_SIZENS : SWT.CURSOR_SIZEWE));
		...
		private Image createMaxButtonImage() {
			...
			Point size = renderer.computeSize(CTabFolderRenderer.PART_MAX_BUTTON, SWT.NONE, tempGC, SWT.DEFAULT, SWT.DEFAULT);
			tempGC.dispose();
		...
		private final class DragListener implements Listener {
			...
			public void handleEvent(Event event) {
				...
				if (dragItem == null) {
					...
					final MainView2 mainView = getParent();
						...
						public void handleEvent(Event event) {
							...
							if (dockComponent == null) {
								...
								if (point.x >= 0 && point.x <= 200 && mainView.west.getItemCount() < 1) {
									...
									int southDockHeight = mainView.getDockHeight(mainView.south);
									tracker.setRectangles(new Rectangle[] {
											new Rectangle(clientArea.width - 200, clientArea.y, 200, clientArea.height - southDockHeight - 2) });
									tracker.setCursor(mainView.dragEast);
								} else if (point.y <= bounds.height && point.y >= bounds.height - 200 && mainView.south.getItemCount() < 1) {
									Rectangle clientArea = mainView.getClientArea();
									tracker.setRectangles(
											new Rectangle[] { new Rectangle(clientArea.x, clientArea.height - 200, clientArea.width, 200) });
					...
					if (tracker.open()) {
						...
						if (dockComponent == null) {
							...
							if (dragItemNewIndex >= 0
									&& (dragTabFolder.getItemCount() <= dragItemNewIndex || dragTabFolder.getItem(dragItemNewIndex) != dragItem)) {
