//  $Id: OVFile.java,v 1.1 2006/01/17 03:01:17 mclark Exp $
/* JOrbis
 ...
 */
//  Based on VorbisFile.java.  Additional code copyright ©2006 Matthew Clark.
...
public class OVFile {
	...
	private SmartStream datasource;
	...
	public OVFile(SmartStream stream, int pcmLength) {
		method = Reflection.getDeclaredMethod(Comment.class, "clear");
		if (pcmLength > 0) {
			pcmlengths = new long[1];
			pcmlengths[0] = pcmLength;
		}
		valid = (this.open_callbacks(stream, null, 0) == 0);
	}

	public OVFile(SmartStream stream) {
		this(stream, 0);
	}

	// get data using read function
		...
		while (i < 3) {
			...
			while (i < 3) {
				...
				if (result == -1) {
					...
					vi.clear();
					// vc.clear();
					Reflection.invokeMethod(method, vc);
					os.clear();
					return -1;
				}
	...
	void prefetch_all_headers(Info first_i, Comment first_c, int dataoffset) {
		...
		for (int i = 0; i < links; i++) {
			...
			/*
			 * { long end=offsets[i+1]; //!!! seek_helper(end);
			 * 
			 * while(true){ ret=get_prev_page(og); if(ret==-1){ // this should not be possible
			 * System.err.println("Could not find last page of logical "+ "bitstream #"+(i)+"\n"); vi[i].clear();
			 * vc[i].clear(); break; } if(og.granulepos()!=-1){ serialnos[i]=og.serialno();
			 * pcmlengths[i]=og.granulepos(); break; } } }
			 */
			...
			if (dataoffsets[i] != -1) {
				...
				while (true) {
					Packet op = new Packet();
					ret = this.get_next_page(og, -1);
					if (ret < 0) // this should not be possible unless file is truncated/mangled
						break;
					if (og.serialno() != serialnos[i])
						break;
					// count blocksizes of all frames in the page
					os.pagein(og);
					while ((result = os.packetout(op)) != 0) {
						if (result > 0) { // ignore holes
							int thisblock = vi[i].blocksize(op);
							if (lastblock != -1)
					...
					if (og.granulepos() != -1) {
						...
						accumulated = op.granulepos - accumulated;
						break;
					}
				}
				// less than zero? this is a stream with samples trimmed off
				// the beginning, a normal occurence; set the offset to zero
				if (accumulated < 0)
					accumulated = 0;
				dataoffsets[i] = accumulated;
				// pcmlengths[i * 2] = accumulated;
			}
			...
			// get the last page of the stream
			{
				...
				while (true) {
					...
					if (ret < 0) { // this should not be possible
						vi[i].clear();
						// vc[i].clear();
						Reflection.invokeMethod(method, vc[i]);
						break;
					}
					if (og.granulepos() != -1) {
						pcmlengths[i] = og.granulepos() - pcmlengths[i * 2];
			...
			/*
			 * // get the PCM length of this link. To do this, // get the last page of the stream { ogg_int64_t
			 * end=vf->offsets[i+1]; _seek_helper(vf,end);
			 * 
			 * while(1){ ret=_get_prev_page(vf,&og); if(ret<0){ // this should not be possible
			 * vorbis_info_clear(vf->vi+i); vorbis_comment_clear(vf->vc+i); break; } if(ogg_page_granulepos(&og)!=-1){
			 * vf->pcmlengths[i*2+1]=ogg_page_granulepos(&og)-vf->pcmlengths[i*2]; break; } vf->offset=ret; } } }
			 */
	...
	int open_seekable() {
		...
		if (pcmlengths == null) {
			...
			// ** shortcut **
			// set offsets and serial #'s for singly-linked stream
			links = 1;
			vi = new Info[links];
			vi[0] = initial_i;
			vc = new Comment[links];
			...
			offsets[0] = 0;
			offsets[1] = datasource.getLength();
			serialnos = new int[links];
			this.raw_seek(0);
			this.get_next_page(og, 0);
			if (og.header_base == null)
				return -1;
			serialnos[0] = og.serialno();
		}
		return (raw_seek(0));
	}
	...
	int open_callbacks(SmartStream is, byte[] initial, int ibytes// , callbacks callbacks
	) {
		int ret;
		datasource = is;
	...
	public int serialnumber(int i) {
		return (i >= 0) ? ((i < links) ? serialnos[i] : -1) : current_serialno;
	}
	...
	public Info getInfo(int link) {
		if (link < 0)
			return decode_ready ? vi[current_link] : null;
		else
			return (link < links) ? vi[link] : null;
	}

	public Comment getComment(int link) {
		if (link < 0)
			return decode_ready ? vc[current_link] : null;
		else
			return (link < links) ? vc[link] : null;
	}
					...
					// a tight loop to pack each size
					{
						...
						if (word == 1) {
							...
							if (bigendianp == 1) {
								...
								// int val;
								/*
								 * for(int j=0;j<samples;j++){ for(int i=0;i<channels;i++){ val=(int)(pcm[i][j]*32768. +
								 * 0.5); if(val>32767)val=32767; else if(val<-32768)val=-32768; val+=off;
								 * buffer[index++]=(byte) (val & 0xff); buffer[index++]=(byte) ((val >> 8) & 0xff); } }
								 */
								for (int i = 0; i < channels; i++) { // note: channels>1 probably don't work
									int ptr = i * 2, mono = _index[i];
									for (int j = 0; j < samples; j++) {
										val = (int) (pcm[i][mono + j] * 32767.);
										if (val > 32767)
											val = 32767;
										else if (val < -32768)
											val = -32768;
